...
export function ReceiptPanel({
  ...
  return (
    <aside className="flex flex-col h-full bg-white" style={{ borderLeft: "1px solid #e2eaef" }}>
      <div className="px-5 py-4 border-b" style={{ borderColor: "#e2eaef" }}>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="font-sans text-[14px] font-bold tracking-wide" style={{ color: "#3c4d59" }}>Live Receipt</h3>
            <p className="font-mono text-[10px] mt-0.5" style={{ color: "#7891a3" }}>
              {items.length === 0 ? "No services selected" : `${items.length} service${items.length !== 1 ? "s" : ""} added`}
            </p>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex items-center rounded-lg p-0.5" style={{ background: "#eef3f6" }}>
              <button
                onClick={() => setCurrency("PHP")}
                className="px-2.5 py-1 rounded-md text-[10px] font-bold transition-all"
                style={currency === "PHP" ? { background: "#fff", color: "#17222c", boxShadow: "0 1px 2px rgba(0,0,0,0.08)" } : { color: "#9db0bc" }}
              >
                PHP
              </button>
              <button
                ...
                className="px-2.5 py-1 rounded-md text-[10px] font-bold transition-all"
                style={currency === "USD" ? { background: "#fff", color: "#17222c", boxShadow: "0 1px 2px rgba(0,0,0,0.08)" } : { color: "#9db0bc" }}
              >
                USD
              </button>
            </div>
            {items.length > 0 && (
              <button
                onClick={onClearAll}
                className="flex items-center justify-center px-2.5 h-7 rounded-lg transition-all duration-150 text-[11px] font-bold"
                style={{ color: "#7891a3" }}
                title="Clear All"
              >
                Clear
        ...
        <input
          ...
          className="w-full px-4 py-2.5 rounded-lg transition-all duration-200 outline-none text-[13px] font-sans"
          style={{ background: "#eef3f6", border: "1px solid #e2eaef", color: "#3c4d59" }}
        />
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-3" ref={receiptRef}>
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-3 py-10">
            <p className="font-sans text-[13px] text-center" style={{ color: "#7891a3" }}>
      ...
      <div className="px-5 pb-5 pt-3" style={{ background: "#eef3f6", borderTop: "1px solid #e2eaef" }}>
        <div className="flex items-center justify-between p-3.5 rounded-xl mb-3" style={{ background: "rgba(77,122,153,0.06)", border: "1px solid rgba(77,122,153,0.18)" }}>
          <div>
            <h4 className="font-bold text-[13px] tracking-tight" style={{ color: "#17222c" }}>First-Time Client</h4>
            <p className="text-[11px] mt-0.5 font-medium" style={{ color: "#4d7a99" }}>Applies a 10% global discount</p>
          </div>
          <button
            onClick={() => onToggleFirstTimeClient(!isFirstTimeClient)}
            className="relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none"
            style={{ background: isFirstTimeClient ? "#4d7a99" : "#cfdce4" }}
          >
            <span
              className="pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out"
              style={{ transform: isFirstTimeClient ? "translateX(20px)" : "translateX(0)" }}
            />
          </button>
        </div>

        <div className="rounded-xl p-4 mb-4 bg-white flex flex-col" style={{ border: "1px solid #e2eaef" }}>
          {isFirstTimeClient && (
            <div className="flex flex-col gap-1.5 mb-3 pb-3 border-b" style={{ borderColor: "#e2eaef" }}>
              <div className="flex justify-between items-center font-sans text-[12px] font-bold" style={{ color: "#7891a3" }}>
                <span>Subtotal</span>
                <span className="font-mono">{currency === "PHP" ? formatPHP(subtotalPhp) : formatUSD(subtotalUsd)}</span>
              </div>
              <div className="flex justify-between items-center font-sans text-[12px] font-bold text-emerald-600">
                <span>10% Discount</span>
                <span className="font-mono">-{currency === "PHP" ? formatPHP(discountPhp) : formatUSD(discountUsd)}</span>
              </div>
            </div>
          )}
          <p className="font-sans text-[10px] font-bold uppercase tracking-widest mb-1.5" style={{ color: "#7891a3" }}>Grand Total</p>
          <div className="font-mono text-[26px] font-bold leading-tight" style={{ color: "#17222c" }}>
            {currency === "PHP" ? formatPHP(totalPhp) : formatUSD(totalUsd)}
          </div>
          <div className="flex items-center gap-1.5 mt-1 font-mono text-[13px]" style={{ color: "#7891a3" }}>
            ≈ <span className="font-semibold" style={{ color: "#3c4d59" }}>{currency === "PHP" ? formatUSD(totalUsd) : formatPHP(totalPhp)}</span>
        ...
        <button
          ...
          className="w-full flex items-center justify-center py-3.5 rounded-xl transition-all duration-200 disabled:opacity-50 text-white font-sans text-[14px] font-semibold"
          style={items.length > 0
            ? { background: "linear-gradient(135deg, #4d7a99, #6a94ad)", boxShadow: "0 4px 14px rgba(77, 122, 153, 0.3)" }
            : { background: "#e2eaef", color: "#9db0bc", cursor: "not-allowed", boxShadow: "none" }}
        >
  ...
  return (
    <div className="flex items-start justify-between gap-2 py-2 group border-b border-transparent transition-colors" style={{ borderBottomColor: "transparent" }}>
      <div className="flex-1 min-w-0">
        <p className="truncate font-sans text-[12px] font-semibold" style={{ color: "#3c4d59" }}>{item.name}</p>
        <p className="font-mono text-[10px] mt-0.5" style={{ color: "#7891a3" }}>{item.detail}</p>
      </div>
      <div className="flex items-center gap-2 shrink-0 mt-0.5">
        <span className="font-mono text-[12px] font-bold" style={{ color: "#4d7a99" }}>{displayPrice}</span>
        <button
          onClick={onRemove}
          className="opacity-0 group-hover:opacity-100 transition-opacity duration-150 px-1.5 h-5 rounded flex items-center justify-center text-[10px] font-bold"
          style={{ color: "#9db0bc" }}
        >
          Remove
        </button>
...
async function exportReceiptAsPNG(
  ...
  if (bannerImg) {
    ...
    ctx.restore();
    ctx.fillStyle = "rgba(13, 20, 32, 0.08)";
  ...
  ctx.fillText("Thank you for choosing Zapolyarny Bureau Piloting Services", CARD_X + (CARD_W / 2), currentY);
  ctx.fillStyle = "#9db0bc";
  ctx.font = "500 10px sans-serif";
  ctx.fillText("@ZBPServices · discord.gg/eqwajc5pea", CARD_X + (CARD_W / 2), currentY + 16);

  const link = document.createElement("a");
  link.download = `ZBP_Receipt_${clientName.replace(/\s+/g, "_")}_${Date.now()}.png`;
